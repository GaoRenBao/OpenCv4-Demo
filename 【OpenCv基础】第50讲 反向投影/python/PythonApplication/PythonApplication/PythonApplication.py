...
def on_BinChange(x):
    ...
    # 获取滑动条的值
    g_bins = cv2.getTrackbarPos('g_bins', 'drawing')
    ...
    for i in range(g_bins):
        cv2.rectangle(histImg, (i * bin_w, h),((int)((i + 1) * bin_w), (int)(h - hist[i][0] * h / 255.0)),(100, 123, 255), -1)
...
cv2.createTrackbar('g_bins', 'drawing', 0, 180, on_BinChange)
cv2.setTrackbarPos('g_bins', 'drawing', 30)

# 显示效果图
cv2.imshow("drawing", g_srcImage)
